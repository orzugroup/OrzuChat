...
create table if not exists public.direct_pairs (
...
create index if not exists calls_participant_status_idx
...
begin
  ...
  values (new.id, generated_username, base_name, new.phone)
...
drop policy if exists "direct_pairs_select_self" on public.direct_pairs;
create policy "direct_pairs_select_self"
  on public.direct_pairs for select to authenticated
...
drop policy if exists "media_insert_own_prefix" on storage.objects;
...
drop policy if exists "media_update_own_prefix" on storage.objects;
